...
kaggle1 <- kaggle %>%
mutate(Relation = as.numeric(kaggle$Relation)) %>%
mutate(PlaceofBirth = as.numeric(kaggle$PlaceofBirth)) %>%
mutate(Topic = as.numeric(kaggle$Topic)) %>% 
mutate(gender = as.numeric(kaggle$gender)) %>%
mutate(StudentAbsenceDays = as.numeric(kaggle$StudentAbsenceDays)) 

numkaggle <- kaggle1 %>%
  dplyr::select(VisITedResources, Relation, PlaceofBirth, Topic, gender, StudentAbsenceDays)

numkaggle = RF_df.norm
```
...
#linearity 
rcorr(as.matrix(numkaggle))

#independence 
plot(numkaggle$residuals, main="Residuals vs Order")
acf(kaggle$residuals)
dwtest(kaggle)
